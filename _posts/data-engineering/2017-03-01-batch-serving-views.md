...
To disable vnodes you need to configure two properties in cassandra.yml
...
Note that the first node in the ring with the lowest initial_token is responsible for the wrapping range (highest initial_token, lowest initial_token]. i.e. the node with the lowest token accepts row keys less than the lowest token and more than the highest token (see [here](https://docs.datastax.com/en/archived/cassandra/1.1/docs/cluster_architecture/partitioning.html#data-distribution-in-the-ring)). 

To take an example: We want 4 nodes with replication factor of 2. This means we have 4 token ranges (minimum allowed with vnodes as each node owns one). Each node will contain its token range and the replica set for another token range (i.e. RF=2). 
